...
@section('title', 'Insert Pelunasan Penjualan')

@section('content_header')
    <h1>Insert Pelunasan Penjualan</h1>
@stop
...
<form method="post" action="/sales-settlement">
    ...
    <div class="form-group d-flex justify-content-end">
        <label for="total-settlement" class="col-md-2 m-1">Nilai Pelunasan</label>
        <input type="input" class="form-control col-md-2 @error('total') is-invalid @enderror" id="total-settlement" name="total" value="0">
    </div>
    @error('total')
...
<script>
    ...
    $('#customer').change(function(){
        $('#total-invoice').val(0);
        $('#total-settlement').val(0);
        $('#details_table tbody').empty();
        if($(this).val()!=0){
            $.ajax({
                method : "GET",
                url : "{{ url('/sales-settlement/getNotSettledSalesInvoice')}}",
                data:{'id':$(this).val()},
                success:function(response){
                    $.each(response, function(index,invoice){
                        ...
                        let row =  '<tr scope="row" class="d-flex">'
                            ...
                            +'<input type="hidden" name="settlementTotals[]" class="form-control " value="'+invoice.settlement_total+'"/>'
                            +'</td>'
                        +'</tr>';
